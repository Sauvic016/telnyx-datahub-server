...
export const getCompletedData = async (filters?: { listName?: string }) => {
  ...
  const contactData = await prisma.contacts.findMany({
    where: whereClause,
    include: {
      directskips: true, // Include DirectSkip to get confirmedAddress
      contact_phones: {
        where: {
          telynxLookupId: { not: null },
        },
        include: {
          telynxLookup: true, // Include TelynxLookup to get caller_id
        }
      },
      property_details: {
        include: {
